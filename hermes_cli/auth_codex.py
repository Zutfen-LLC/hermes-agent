...
def _codex_routable_pool_dicts() -> Iterator[Dict[str, Any]]:
    """Codex pool rows whose credential the route that would receive it accepts. The ChatGPT Codex
    endpoint rejects API keys, so an ``api_key`` row routed there is never handed out (or reported) as
    the subscription credential (ops-supervisor#216); one routed to a custom Codex gateway is that
    gateway's own key and stays usable (#121486)."""
    from hermes_cli.auth import read_credential_pool
    canonical = DEFAULT_CODEX_BASE_URL.rstrip("/")
    return (e for e in _codex_pool_dicts(read_credential_pool("openai-codex"))
            if e.get("auth_type") != "api_key"
            or _codex_pool_route_base_url(e.get("base_url")).rstrip("/") != canonical)
...
def _codex_pool_rate_limit_status() -> Optional[Dict[str, Any]]:
    ...
    try:
        now = time.time()
        for entry in _codex_routable_pool_dicts():
